...
databaseFiles = os.listdir('C:\\wamp\\www\\Database\\')
...
if len(backupFolders) == 0:
	...
	for dataFile in databaseFiles:
			...
			#Case 2: cIndex is not -1, so it exists in the database
				...
				if time.strftime("%Y-%m-%dT%H-%M-%S", time.localtime(os.path.getmtime('C:\\wamp\\www\\Database\\' + dataFile))) > refs[i].split(',')[1]:
					filesToBackup.append(dataFile)
...
if len(filesToBackup) > 0:
	...
	for fileToBackup in filesToBackup:
		shutil.copy2('C:\\wamp\\www\\Database\\' + fileToBackup, backupFolder + '\\' + fileToBackup)
		
	#Save a new index file to the new backup folder
	f = open(backupFolder + '\\index.txt', 'wb')
	for dataFile in databaseFiles:
		modifiedDate = time.strftime("%Y-%m-%dT%H-%M-%S", time.localtime(os.path.getmtime('C:\\wamp\\www\\Database\\' + dataFile)))
