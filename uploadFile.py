...
wb_uploaded = openpyxl.load_workbook(uploadDIR + filename)
...
if os.path.getsize(location) <= os.path.getsize(databaseDIR + filename):
	print "ERROR: Uploaded file has a size smaller than or equal to the original file."
	sys.exit(1)
...
if len(wb_original.worksheets) != len(wb_uploaded.worksheets):
	print "ERROR: Uploaded file does not have same number of sheets as original file."
	sys.exit(1)
...
#Now redo the permissions of the file so that it is not read-only
