...
if(isset($_GET["workbook"]) and isset($_GET["sheet"]) and isset($_GET["row"]) and isset($_GET["col"]) and isset($_GET["number"]) and isset($_GET["sourceID"])) {
	...
	echo 'Workbook: ' . $_GET['workbook'] . "<br>Sheet: " . $_GET["sheet"] . '<br>Row: ' . $_GET['row'] . '<br>Column: ' . $_GET['col'] . '<br>Number Value: ' . $_GET['number'] . '<br>SourceID: <a href="http://141.218.60.56/~jnz1568/getSource.php?sourceID=' . $_GET['sourceID'] . '">' . $_GET['sourceID'] . '</a><br>Blog Link: Coming soon';
